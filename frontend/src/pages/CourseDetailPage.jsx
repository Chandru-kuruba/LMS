...
export default function CourseDetailPage() {
        ...
        const fetchCourse = async () => {
            ...
            try {
                ...
                if (isAuthenticated && accessToken) {
                    try {
                        const enrolledRes = await axios.get(`${API}/enrolled-courses`, {
                            headers: { Authorization: `Bearer ${accessToken}` }
                        });
                        const enrolled = enrolledRes.data.courses?.some(c => c.id === courseId);
                        setIsEnrolled(enrolled);
                        
                        // Check if user has already reviewed
                        if (enrolled) {
                            const reviewsRes = await axios.get(`${API}/courses/${courseId}`);
                            // We'd need a backend endpoint to check user's own review
                            // For now, we'll handle "already reviewed" error on submit
                        }
                    } catch (e) {
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            await axios.post(
                `${API}/courses/${courseId}/reviews?rating=${reviewRating}&comment=${encodeURIComponent(reviewComment)}`,
                null,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            );
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (navigator.share) {
                                                try {
                                                    await navigator.share({
                                                        title: course.title,
                                                        text: course.short_description || `Check out this course: ${course.title}`,
                                                        url: shareUrl
                                                    });
                                                    return;
                                                } catch (e) {
                                            ...
                                            try {
                                                ...
                                                try {
                                                    ...
                                                    toast.error("Press Ctrl+C to copy: " + shareUrl);
                                                }
                                                document.body.removeChild(textArea);
                                            }
                                        }}
                                        data-testid="share-course-btn"
                                    >
                    ...
                    {activeTab === "reviews" && (
                        ...
                        >
                            ...
                            {isEnrolled && !hasReviewed && (
                                <div className="glass-heavy rounded-xl p-6 mb-8 border border-purple-500/30">
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-slate-400 ml-2">{reviewRating}/5</span>
                                        </div>
                                    </div>
                            ...
                            {isEnrolled && hasReviewed && (
                                <div className="glass-light rounded-xl p-4 mb-8 border border-green-500/30">
                            ...
                            {!isEnrolled && isAuthenticated && (
                                <div className="glass-light rounded-xl p-4 mb-8 border border-slate-500/30">
