...
export default function SettingsPage() {
    ...
    const fetchAllSettings = async () => {
        ...
        try {
            ...
            // Fetch bucket usage after getting buckets
            if (bucketsRes.data.buckets?.length > 0) {
                fetchBucketUsage();
            }
        } catch (error) {
    ...
    // Fetch storage usage for all buckets
    const fetchBucketUsage = async () => {
        setLoadingUsage(true);
        try {
            const res = await axios.get(
                `${API}/admin/settings/r2-buckets/usage/all`,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            );
            const usageMap = {};
            res.data.buckets.forEach(b => {
                usageMap[b.bucket_id] = b;
            });
            setBucketUsage(usageMap);
        } catch (error) {
            console.error("Failed to fetch bucket usage:", error);
    ...
    // Fetch usage for a single bucket
    const fetchSingleBucketUsage = async (bucketId) => {
        try {
            const res = await axios.get(
                `${API}/admin/settings/r2-buckets/${bucketId}/usage`,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            );
            setBucketUsage(prev => ({ ...prev, [bucketId]: res.data }));
        } catch (error) {
            console.error("Failed to fetch bucket usage:", error);
        }
    };
                ...
                <TabsContent value="r2-buckets" className="space-y-4">
                    ...
                    <div className="grid gap-4">
                        {buckets.length === 0 ? (
                            ...
                            buckets.map((bucket) => {
                                const usage = bucketUsage[bucket.id];
                                return (
                                    <div key={bucket.id} className="glass-card p-4 rounded-xl">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                                                    usage?.is_over_limit ? 'bg-red-500/20' : 
                                                    usage?.is_near_limit ? 'bg-yellow-500/20' : 'bg-orange-500/20'
                                                }`}>
                                                    <Cloud className={`w-6 h-6 ${
                                                        usage?.is_over_limit ? 'text-red-400' : 
                                                        usage?.is_near_limit ? 'text-yellow-400' : 'text-orange-400'
                                                    }`} />
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Button variant="outline" size="sm" onClick={() => fetchSingleBucketUsage(bucket.id)}>
                                                    <HardDrive className="w-4 h-4 mr-1" /> Usage
                                                </Button>
                                            ...
                                            <div className="mt-3 pt-3 border-t border-slate-700/50">
                                                <div className="flex items-center justify-between mb-2">
                                                    ...
                                                    <span className={`text-sm font-medium ${
                                                        usage.is_over_limit ? 'text-red-400' : 
                                                        usage.is_near_limit ? 'text-yellow-400' : 'text-green-400'
                                                    }`}>
                                                        {usage.total_size_gb >= 1 
                                                            ? `${usage.total_size_gb} GB` 
                                                            : `${usage.total_size_mb} MB`} / {usage.limit_gb} GB
                                                    </span>
                                                </div>
                                                <Progress 
                                                    value={Math.min(usage.usage_percent, 100)} 
                                                    className={`h-2 ${
                                                        usage.is_over_limit ? '[&>div]:bg-red-500' : 
                                                        usage.is_near_limit ? '[&>div]:bg-yellow-500' : '[&>div]:bg-green-500'
                                                    }`}
                                                />
                                                <div className="flex justify-between mt-2 text-xs text-slate-500">
                                                    <span>{usage.total_objects} files</span>
                                                    <span>{usage.usage_percent}% used</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {usage?.error && (
                                            <div className="mt-3 pt-3 border-t border-slate-700/50">
                                                <p className="text-sm text-red-400 flex items-center gap-2">
                                                    ...
                                                    Failed to fetch usage: {usage.error}
                                                </p>
                                            </div>
                                        )}
                                    </div>
                    ...
                    {buckets.length > 0 && (
                        <div className="flex justify-center pt-4">
                            <Button 
                                variant="outline" 
                                onClick={fetchBucketUsage}
                                disabled={loadingUsage}
                            >
                                <RefreshCw className={`w-4 h-4 mr-2 ${loadingUsage ? 'animate-spin' : ''}`} />
                                {loadingUsage ? 'Loading...' : 'Refresh All Usage Stats'}
                            </Button>
                        </div>
                    )}
                </TabsContent>
