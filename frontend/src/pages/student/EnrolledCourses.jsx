...
import { BookOpen, Play, Award, Clock, Search, Filter, Star, Send } from "lucide-react";
...
export default function EnrolledCoursesPage() {
    ...
    const openReviewDialog = (course, e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            await axios.post(
                `${API}/courses/${reviewCourse.id}/reviews?rating=${reviewRating}&comment=${encodeURIComponent(reviewComment)}`,
                null,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            );
            toast.success("Thank you for your review!");
            setReviewedCourses({ ...reviewedCourses, [reviewCourse.id]: true });
                                    ...
                                    <div className="flex items-center justify-between text-sm text-slate-500">
                                        ...
                                        {!reviewedCourses[course.id] && (
                                            <Button
                                                ...
                                                onClick={(e) => openReviewDialog(course, e)}
                                                data-testid={`write-review-${course.id}`}
                                            >
            ...
            <Dialog open={showReviewDialog} onOpenChange={setShowReviewDialog}>
                <DialogContent className="glass-heavy border-purple-500/30">
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            <Label className="text-slate-300">Course</Label>
                            <p className="text-white font-medium">{reviewCourse?.title}</p>
                        </div>
